...
def reed_pand(bias_char):
    sampl = ["balanced", "stratfied", "residual"] #sampling method
    sampl_num = ["balanced_bins", "stratfied_bins" , "resudual_bins"]
    for sampling in sampl:
        ...
        plt.xlabel('Data');
        plt.title('Sampling Method: ' + sampling + ' Bias Character: ' + bias_char)
        plt.show()
def reed_pand_bins(bias_char):
    # sampl = ["balanced", "stratfied", "resudual"] #sampling method
    sampl_num = ["balanced_bins", "stratfied_bins" , "residual_bins"]
    for sampling in sampl_num:
        df_train = pd.read_csv('../../data/txt_files/' + sampling + "/" + bias_char + "/" + 'Train.csv', delimiter=',',
                               encoding='utf-8', header=None)
        df_train.columns = ["A", "B", "C"]
        df_val = pd.read_csv('../../data/txt_files/' + sampling + "/" + bias_char + "/" + 'Val.csv', delimiter=',',
