...
def filter(file_path:str,bias_char:str):
    ...
    # trim['bins'] = pd.cut(trim[bias_char], bins=bins, include_lowest=True)
    labels = ["{0} - {1}".format(i, i + ((max_value-min_value))/3) for i in np.arange((min_value), (max_value),((max_value-min_value)/3))]
    zcd = np.arange((min_value - 1),( max_value + 1),((max_value-min_value)/3))
    trim["bins"] = pd.cut(trim[bias_char], zcd , include_lowest=True)
