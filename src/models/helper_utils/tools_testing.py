...
def obtain_label(loader, model,class_num):
    epsilon = 1e-5
    start_test = True
    with torch.no_grad():
        ...
        for _ in range(len(loader)):
            ...
            feas, raw_outputs = model(inputs)
            outputs = nn.Softmax(dim=1)(raw_outputs)
            _, predict = torch.max(outputs, 1)


            # if predict==class_num:
            ...
            if start_test:
                ...
                all_output = torch.cat((all_output, outputs.float().cpu()), 0)
                all_label = torch.cat((all_label, labels.float()), 0)
                all_lab = torch.cat((all_lab, lab.float()), 0)
                all_path = all_path + paths

    from sklearn.decomposition import PCA
    pca = PCA(2)  # we need 2 principal components.
    converted_data = pca.fit_transform(all_fea)
    all_fea = converted_data

    from sklearn.cluster import KMeans
    kmeans = KMeans(1, random_state=0).fit(all_fea)
    labels = kmeans.predict(all_fea)
    X_dist = kmeans.transform(all_fea) ** 2
    all_feas = converted_data
    plt.style.use('seaborn-whitegrid')
    plt.figure(figsize=(10, 6))
    c_map = plt.cm.get_cmap('jet', 2)
    plt.scatter(all_feas[:, 0], all_feas[:, 1], c=all_lab, s=50, cmap='viridis')
    centers = kmeans.cluster_centers_
    plt.scatter(centers[:, 0], centers[:, 1], c='black', s=200, alpha=0.5)
    # plt.show()
    plt.scatter(all_feas[:, 0], all_feas[:, 1], c=labels, s=50, cmap='viridis')
    ...
    tsne_result = tsne.fit_transform(all_fea)
    tsne_result_df = pd.DataFrame({'tsne_1': tsne_result[:, 0], 'tsne_2': tsne_result[:, 1], 'label': all_lab})
    ...
    return all_label, labels , all_path ,X_dist


def obtain_cat(loader, model,num_classes):
    epsilon = 1e-5
    start_test = True
    with torch.no_grad():
        iter_test = iter(loader)
